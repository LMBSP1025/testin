...
// 메인 배너 설정 가져오기
async function getMainBannerPost() {
  try {
    // 서버 사이드에서 직접 Gist API 호출 (SSO 우회)
    const selectedBannerSlug = await getMainBannerFromGist();
    
    console.log('Gist에서 가져온 배너 슬러그:', selectedBannerSlug);
    
    if (!selectedBannerSlug) {
      console.log('메인 배너 설정이 없습니다');
      return null;
    }
    
    // 선택된 배너 게시글 가져오기
    const bannerPost = await getPostBySlug(selectedBannerSlug);
    console.log('배너 게시글:', bannerPost ? bannerPost.title : '없음');
    
    return bannerPost;
  } catch (error) {
    console.error('메인 배너 로드 실패:', error);
    return null;
  }
}

export default async function Index() {
  const bannerPost = await getMainBannerPost();
  
  return (
    <main className="relative flex flex-col items-center justify-center min-h-screen py-12 px-5 mb-8">
      {/* 윈도우 98 스타일 창 */}
      <div className="max-w-4xl mx-auto shadow-[2px_2px_0_0_#000000] bg-gray-300">
        ...
        <div className="bg-blue-900 border-2 border-gray-300 text-white px-3 py-1 ">
          <h2 className="text-xl font-bold">메인</h2>
        </div>
        
        <div className="p-4">
          {bannerPost ? (
            <div className="">
              {/* 배너 게시글의 이미지 표시 (images 필드 또는 마크다운에서 추출) */}
              {(bannerPost.images && bannerPost.images.length > 0) ? (
                <div className="mb-4 flex justify-center">
                  <img 
                    src={bannerPost.images[0]} 
                    alt={bannerPost.title}
                    className="w-full md:w-[500px] lg:w-[600px] h-auto"
                  />
                </div>
              ) : (
                // 마크다운 내용에서 이미지 URL 추출 시도
                (() => {
                  const imageMatch = bannerPost.content.match(/!\[.*?\]\((.*?)\)/);
              ...
              {/* 제목을 이미지 아래로 이동하고 크기 조정 */}
              <h1 className="text-lg font-medium text-black mb-1">{bannerPost.title}</h1>
              
              {bannerPost.excerpt && (
                <p className="text-gray-700 text-sm">{bannerPost.excerpt}</p>
              )}
              ...
              {/* 배너 게시글로 이동하는 링크는 제거 (요청사항) */}
            </div>
          ) : (
            <div className="">
              <h1 className="text-2xl font-bold text-black mb-4">환영합니다!</h1>
              <p className="text-gray-700 mb-5">설정된 메인 배너가 없습니다.</p>
            </div>
          )}
          <div className="">
